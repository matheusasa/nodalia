...
export default function Admin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await fetch(`/api/publicacoes/${editingId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      } else {
        await fetch('/api/publicacoes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      }
      resetForm();
      fetchPubs();
    ...
    <div className="pt-28 pb-16 min-h-screen bg-bg-secondary">
      <div className="max-w-[1000px] mx-auto px-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-heading text-3xl font-extrabold text-text-primary">Publicações</h1>
            <p className="text-text-secondary text-sm mt-1">Gerencie seus cases e artigos</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-border-subtle p-8 mb-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Título da publicação"
              ...
              <div>
                ...
                <textarea required rows={2} value={form.resumo} onChange={(e) => setForm({ ...form, resumo: e.target.value })} placeholder="Resumo curto da publicação"
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5">Conteúdo (Markdown) *</label>
                <textarea required rows={10} value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} placeholder="Escreva o conteúdo em Markdown..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <input type="text" value={form.imagem} onChange={(e) => setForm({ ...form, imagem: e.target.value })} placeholder="https://..."
                    className="w-full px-4 py-3 rounded-xl bg-bg-secondary border border-border-subtle text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent-start focus:ring-1 focus:ring-accent-start/30 transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1.5">Tags (separar por vírgula)</label>
                  <input type="text" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} placeholder="automação, dados, whatsapp"
                    className="w-full px-4 py-3 rounded-xl bg-bg-secondary border border-border-subtle text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent-start focus:ring-1 focus:ring-accent-start/30 transition-all" />
                </div>
              </div>
              <div className="flex items-center gap-3">
                <input type="checkbox" id="published" checked={form.published} onChange={(e) => setForm({ ...form, published: e.target.checked })} className="w-4 h-4 accent-accent-dark" />
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-border-subtle">
            ...
            <p className="text-text-muted text-sm mt-1">Certifique-se de que o banco MySQL está configurado e as migrations foram aplicadas.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {pubs.map((pub) => (
              <div key={pub.id} className="flex items-center justify-between p-5 bg-white rounded-xl border border-border-subtle shadow-sm hover:border-border-hover transition-all">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-heading font-bold text-text-primary truncate">{pub.title}</h3>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${pub.published ? 'bg-accent-start/10 text-accent-dark' : 'bg-orange-100 text-orange-600'}`}>
                      {pub.published ? 'Publicado' : 'Rascunho'}
                    </span>
                  </div>
                  <p className="text-text-muted text-sm truncate">{pub.resumo}</p>
                </div>
                <div className="flex items-center gap-2 ml-4">
                  <button onClick={() => handleEdit(pub)} className="p-2 rounded-lg hover:bg-bg-secondary text-text-muted hover:text-text-primary transition-all cursor-pointer" title="Editar">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                  </button>
                  <button onClick={() => handleDelete(pub.id)} className="p-2 rounded-lg hover:bg-red-50 text-text-muted hover:text-red-500 transition-all cursor-pointer" title="Excluir">
